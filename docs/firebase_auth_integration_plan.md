...
- Create `lib/features/auth/data/firebase_auth_service.dart` wrapping the Firebase Auth SDK (sign-in, sign-up, sign-out, password reset, auth state).
...
- Add Riverpod providers:
  - `authStateChangesProvider` streaming `AsyncValue<User?>` via `FirebaseAuth.instance.authStateChanges()`.
